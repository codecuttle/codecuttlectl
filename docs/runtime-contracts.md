...
This is the R1 test/documentation slice of runtime convergence, measured against
merged main `4bcf563` (#67). It changes no production runtime behavior.
...
| C11 | `TestKnownDivergence_EngineEmitsTwoTerminalErrors`: one permanent provider error yields two EventTurnDone errors | One terminal outcome (R3); full-mailbox loss/blocking not exercised in this slice |
| C12 | Not exercised: concurrent Engine admission with a mutable real Agent would intentionally race | Add a safe turn-runner seam and ownership regression in R2, not a racy green test |
...
fixed. Remaining measurement gaps above are explicit. R2 must add ownership tests
without depending on an already-merged ownership fix; R3 must cover saturated
mailboxes; R6–R9 cannot cut over functionality on the strength of this baseline.
